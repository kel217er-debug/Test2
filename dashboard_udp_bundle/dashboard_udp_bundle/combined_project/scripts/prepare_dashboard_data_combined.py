...
- вход: `lm/Заявки+обращения_объединено.xlsx`
...
from dashboard_logic.combined_columns import canonicalize_combined_dataframe
...
from dashboard_logic.excel_filters_combined import (
    ...
    is_base_service,
    is_connected,
    is_employee_tab_excluded_april_2026_federal_channel,
    is_employee_tab_excluded_service,
    is_internet_service,
    is_our_channel,
    is_transferred,
)
...
from calc_combined_mapping_fields import (  # type: ignore
    _read_mapping,
    _required_source_columns,
    _validate_required_mapping_sources,
    build_fields,
)
...
def _load_combined_exclusions(cfg_path):
    cfg = {'employees': []}
    ...
    try:
        ...
        if isinstance(raw, dict) and isinstance(raw.get('employees'), list):
            cfg['employees'] = [clean_name(v) for v in raw['employees'] if clean_name(v)]
    except Exception as e:
        print(f"[ETL COMBINED] WARN: failed to read exclusions config {cfg_path}: {e}", file=sys.stderr)
    return cfg


def _is_excluded_employee(name, excluded_names):
    if not name or not excluded_names:
        return False
    name_clean = clean_name(name)
    if not name_clean:
        return False
    name_tokens = {t.lower() for t in re.findall(r"[A-Za-zА-Яа-яЁё]+", name_clean)}
    for excluded in excluded_names:
        excluded_clean = clean_name(excluded)
        if not excluded_clean:
            continue
        if name_clean == excluded_clean or name_clean.startswith(excluded_clean):
            return True
        excluded_tokens = [t.lower() for t in re.findall(r"[A-Za-zА-Яа-яЁё]+", excluded_clean)]
        if excluded_tokens and all(t in name_tokens for t in excluded_tokens):
            return True
    return False


def _load_excluded_combined_rows(xlsx_path):
    if not xlsx_path or not os.path.exists(xlsx_path):
        return {
            'appeals_from_obrash': set(),
            'rows_count': 0,
        }
    ...
    for _, row in df.iterrows():
        # Prefer the explicit "Номер обращения" column in the exclusions registry.
        # Keep fallback to the legacy "Обращ" name for backward compatibility.
        obrash = _norm_exact_match(row.get('Номер обращения')) or _norm_exact_match(row.get('Обращ'))
        if obrash:
            result['appeals_from_obrash'].add(obrash)

    return result


def _is_excluded_combined_row(row, excluded_keys):
    # In the merged workbook the incident-side identifier lives in
    # "Обращение.Номер обращения". Keep the old field as a fallback
    # for legacy exports.
    appeal = _norm_exact_match(row.get('Обращение.Номер обращения')) or _norm_exact_match(row.get('Номер обращения'))
    return bool(appeal) and appeal in excluded_keys['appeals_from_obrash']


def _build_rows_from_combined(input_xlsx: str, mapping_txt: str, excluded_rows_xlsx: str | None = None):
    ...
    df_in, resolved_columns = canonicalize_combined_dataframe(df_in, required_columns=required_columns)
    _validate_required_mapping_sources(df_in, mapping_rows)
    excluded_keys = _load_excluded_combined_rows(excluded_rows_xlsx)
    excluded_mask = df_in.apply(lambda row: _is_excluded_combined_row(row, excluded_keys), axis=1)
    excluded_source_count = int(excluded_mask.sum())
    if excluded_source_count:
        df_in = df_in.loc[~excluded_mask].reset_index(drop=True)
    ...
    return (
        ...
        excluded_keys.get('rows_count', 0),
        resolved_columns,
    )
...
def _teamlead_selected(teamlead, include_teamleads=None, exclude_teamleads=None):
    if include_teamleads is not None and teamlead in include_teamleads:
        return True
    if exclude_teamleads is not None and teamlead in exclude_teamleads:
        return False
    return None
...
def run_etl_combined(input_xlsx: str, mapping_txt: str, hierarchy_path: str, out_path: str, include_mrfs=None, exclude_mrfs=None):
    ...
    include_teamleads = _env_list('DASH_INCLUDE_TEAMLEADS')
    exclude_teamleads = _env_list('DASH_EXCLUDE_TEAMLEADS')
    exclusions_cfg_path = os.path.join(os.path.dirname(hierarchy_path), 'combined_source_exclusions.json')
    exclusions_cfg = _load_combined_exclusions(exclusions_cfg_path)
    excluded_employees = set(exclusions_cfg.get('employees', []))
    excluded_rows_xlsx = os.path.join(os.path.dirname(os.path.dirname(input_xlsx)), 'Del', 'исключаемые заявки и обращения.xlsx')
    ...
    (
        ...
        repeat_comments,
        excluded_source_count,
        excluded_registry_count,
        resolved_columns,
    ) = _build_rows_from_combined(
        input_xlsx,
        mapping_txt,
        excluded_rows_xlsx=excluded_rows_xlsx,
    )
    ...
    n_read = 0; n_selected = 0; n_our = 0; n_skip_no_date = 0; n_excluded = 0
    ...
    for row in rows_iter:
        ...
        _include_emp_tab_req_conn = not is_employee_tab_excluded_service(row[COL['service']])
        ...
        equip_nd = 0.0 if equip_price > ND_OUTLIER_CAP else equip_price
        nd_val = inst + mon_ + equip_nd
        ...
        if _is_excluded_employee(primary_name, excluded_employees) or _is_excluded_employee(exec_name, excluded_employees):
            n_excluded += 1
            continue
        ...
        teamlead_override = _teamlead_selected(
            teamlead,
            include_teamleads=include_teamleads,
            exclude_teamleads=exclude_teamleads,
        )
        if teamlead_override is False:
            continue
        if teamlead_override is not True and not _mrf_selected(mrf, include_mrfs, exclude_mrfs):
            continue
        n_selected += 1
        ...
        if _include_emp_tab_req_conn and is_employee_tab_excluded_april_2026_federal_channel(
            reg_dt,
            row[COL['channel']],
            exec_mrf,
        ):
            _include_emp_tab_req_conn = False
        ...
        if exec_name:
            ...
            if _connected:
                weekly_reg[wkey_reg]['employee_exec'][exec_name]['nd_sum_created'] += nd_val
                monthly_reg[mkey_reg]['employee_exec'][exec_name]['nd_sum_created'] += nd_val
            if _include_emp_tab_req_conn:
                weekly_reg[wkey_reg]['employee_exec'][exec_name]['n_emp_tab_total'] += 1
                monthly_reg[mkey_reg]['employee_exec'][exec_name]['n_emp_tab_total'] += 1
                if _connected:
                    weekly_reg[wkey_reg]['employee_exec'][exec_name]['n_emp_tab_connected'] += 1
                    monthly_reg[mkey_reg]['employee_exec'][exec_name]['n_emp_tab_connected'] += 1
        ...
        if _has_final:
            if final_dt is not None:
                ...
                # For lagging employee metrics, attribute closed rows by the
                # assigned executor ("ФИО исполнителя"), not by the primary
                # employee who first accepted the request.
                if exec_name:
                    apply_closed(weekly_closed[wkey_cl]['employee'][exec_name])
                    apply_closed(monthly_closed[mkey_cl]['employee'][exec_name])
    ...
    print(f"[ETL COMBINED] Read {n_read}, our={n_our}, excluded={n_excluded}, skip_no_date={n_skip_no_date}, {time.time()-t0:.1f}s")
    ...
    out = {
        'meta': {
            ...
            'excluded_rows_file': os.path.relpath(excluded_rows_xlsx, os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) if os.path.exists(excluded_rows_xlsx) else None,
            'exclusions_file': os.path.relpath(exclusions_cfg_path, os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
            ...
            'n_rows_excluded_by_registry': excluded_source_count,
            'n_rows_in_exclusion_registry': excluded_registry_count,
            'resolved_column_aliases': {k: v for k, v in sorted(resolved_columns.items()) if v and v != [k]},
...
def main():
    ...
    input_xlsx = os.path.join(root, 'lm', 'Заявки+обращения_объединено.xlsx')
