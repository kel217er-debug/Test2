...
from dashboard_logic.excel_filters import (
    ...
    is_base_service,
    is_connected,
    is_excluded_muz_rd_center_nod_service,
...
def run_etl(muz_path, hierarchy_path, out_path, include_mrfs=None, exclude_mrfs=None):
    t0 = time.time()
    ...
    n_read = 0; n_selected = 0; n_our = 0; n_skip_no_date = 0; n_excluded_special = 0
    ...
    for row in rows_iter:
        ...
        if is_excluded_muz_rd_center_nod_service(row[COL['mrf']], podr_prim, row[COL['service']]):
        ...
        def apply_clean_primary_connected(m):
        ...
        apply_clean_primary_connected(weekly_reg[wkey_reg]['total']['all'])
        if _connected and primary_name and exec_name:
        ...
        apply_clean_primary_connected(monthly_reg[mkey_reg]['total']['all'])
        if _connected and primary_name and exec_name:
        ...
        if direction:
            ...
            apply_clean_primary_connected(weekly_reg[wkey_reg]['direction'][direction])
            apply_clean_primary_connected(monthly_reg[mkey_reg]['direction'][direction])
        if exec_direction:
        ...
        if director:
            ...
            apply_clean_primary_connected(weekly_reg[wkey_reg]['director'][director])
            apply_clean_primary_connected(monthly_reg[mkey_reg]['director'][director])
        if exec_director:
        ...
        if teamlead:
            ...
            apply_clean_primary_connected(weekly_reg[wkey_reg]['teamlead'][teamlead])
            apply_clean_primary_connected(monthly_reg[mkey_reg]['teamlead'][teamlead])
        if exec_teamlead:
        ...
        if primary_name:
            ...
            apply_clean_primary_connected(weekly_reg[wkey_reg]['employee'][primary_name])
            apply_clean_primary_connected(monthly_reg[mkey_reg]['employee'][primary_name])
        if exec_name:
        ...
        if _has_final:
            if final_dt is not None:
                ...
                if exec_director:
                    apply_closed(weekly_closed[wkey_cl]['director'][exec_director])
                    apply_closed(monthly_closed[mkey_cl]['director'][exec_director])
                if exec_teamlead:
                    apply_closed(weekly_closed[wkey_cl]['teamlead'][exec_teamlead])
                    apply_closed(monthly_closed[mkey_cl]['teamlead'][exec_teamlead])
                # For MUZ lagging metrics on the employee hierarchy, attribute closed
                # rows by the assigned executor ("ФИО исполнителя"), not by the
                # primary employee who first accepted the request.
                if exec_name:
    ...
    print(
        f"[ETL v2] Read {n_read}, our={n_our}, excluded_special={n_excluded_special}, "
        f"skip_no_date={n_skip_no_date}, {time.time()-t0:.1f}s"
    )
    ...
    out = {
        'meta': {
            ...
            'n_rows_our': n_our,
            'n_rows_excluded_special': n_excluded_special,
...
def main():
    ...
    run_etl(muz, hierarchy, out, include_mrfs=_env_list('DASH_INCLUDE_MRFS'), exclude_mrfs=_env_list('DASH_EXCLUDE_MRFS'))
    return
