...
def new_reg_metrics():
    return {
        ...
        # Employee tab counters: same exec attribution as n_conv_exec_*,
        # but with additional service exclusions applied only for the UI
        # columns "Заявки" and "Подкл." on the employees tab.
        'n_emp_tab_total': 0,
        'n_emp_tab_connected': 0,
        # Clean conversion: primary == exec (same employee).
        'n_conv_clean_total': 0,
        'n_conv_clean_connected': 0,
        'n_month_conv_clean_total': 0,
        'n_month_conv_clean_connected': 0,
        # Legacy fields (kept for backward-compat with older dashboards / saved JSON).
